...
class SiteFooter extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const form = this.querySelector('#footer-newsletter');
    if (form) {
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const email = this.querySelector('#footer-newsletter-email').value.trim();
        if (email) {
          alert(`Thank you for subscribing! Safety updates will be sent to: ${email}`);
          form.reset();
        }
      });
    }
